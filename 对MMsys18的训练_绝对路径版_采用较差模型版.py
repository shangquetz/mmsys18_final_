...
for b in a.iterdir():
    ...
    if b.suffix in ('.csv', '.txt'):
        ...
        os.chdir(fr"D:\MMsys18_final\save_model\{model_folder_name}")
        for index in range(5):
            print(f'现在对{index + 1}s进行预测')

            os.chdir(fr"D:\MMsys18_final\save_model\{model_folder_name}\model_save_{index + 1}s")
            # 真实值采用滑动窗口对齐数据规模
        ...
        plt.savefig(fr"D:\MMsys18_final\save_figure\{b.stem}_figure.png")
        plt.show()
...
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig(fr"D:\MMsys18_final\save_figure\sum_figure.png")
plt.show()
